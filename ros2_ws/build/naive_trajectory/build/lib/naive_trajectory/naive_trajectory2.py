...
class AdaptiveCorridorFollower(Node):
    def __init__(self):
        ...
        self.BASE_SPEED = 110
        self.MIN_SPEED = 110  # VITESSE CONSTANTE - Ne descend jamais en dessous
        self.MAX_SPEED = 110  # VITESSE CONSTANTE
        self.TURN_SPEED_REDUCTION = 0.9  # PAS de réduction en virage
        ...
        self.STUCK_TIME_LIMIT = 1.0  # Temps d'exploration
    ...
    def check_path_clear(self, x1, y1, x2, y2, num_checks=8):
        ...
        for i in range(num_checks):
            ...
            x = x1 + t * (x2 - x1)
            y = y1 + t * (y2 - y1)
            if self.get_cell_value(x, y) > 40:  # PLUS STRICT - détecte obstacles plus tôt
                return False
        return True
    ...
    def is_point_safe(self, x, y, radius=None):
        ...
        # Check du point central (PLUS STRICT)
        if self.get_cell_value(x, y) > 40:
            return False
        
        # Check circulaire autour du point (PLUS DE CHECKS)
        num_checks = 12  # Augmenté de 8 à 12 pour plus de précision
        for i in range(num_checks):
            angle = 2 * math.pi * i / num_checks
            check_x = x + radius * math.cos(angle)
            check_y = y + radius * math.sin(angle)
            if self.get_cell_value(check_x, check_y) > 40:  # PLUS STRICT
                return False
    ...
    def find_best_gap(self, rx, ry, heading):
        ...
        for scan_dist in self.SCAN_DISTANCES:
            ...
            for idx, angle in enumerate(angles):
                ...
                if cell_value > 40:  # PLUS STRICT - évite les murs plus tôt
    ...
    def control_loop(self):
        ...
        # VITESSE FIXE À 110 PWM - Aucune variation
        self.current_speed = 110
